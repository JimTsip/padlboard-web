...
Static site (no build step), deployed via GitHub Pages. There is no form on the page any more — the app is out, so the hero links to the App Store and `hello@padlboard.ai`. The backend lives on Cloudflare (`padlboard-api`, see the app repo's `backend/`).
